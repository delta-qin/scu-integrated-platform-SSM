...
import com.deltaqin.scussm.service.FollowService;
...
public class UserController implements CommunityConstant {
    ...
    // 注意文件名是在打开settings页面的时候就随机生成的，并且获取到了客户端提交的凭证
    // 此时客户端可以拿到Model里面的两个值，直接上传即可，回调知道成功，就给后端通知，更新缓存
    // 返回的页面里面的Model包含了上传凭证
    @LoginRequired
    @RequestMapping(path = "/setting", method = RequestMethod.GET)
    public String getSettingPage(Model model) {
        // 上传文件名称（显示以前的足迹  以及  解决服务器的缓存）
        // 所以使用的是随机数
    ...
    public String updateHeaderUrl(String fileName) {
        if (StringUtils.isBlank(fileName)) {
            return JSONStringUtil.getJSONString(1, "文件名不能为空!");
        }
