...
import com.deltaqin.scussm.dao.elasticsearch.DiscussPostRepository;
...
public class ElasticsearchTests {
    ...
    public void testSearchByRepository() {
        ...
        SearchQuery searchQuery = new NativeSearchQueryBuilder()
                // 搜索条件：多个字段同时匹配
                .withQuery(QueryBuilders.multiMatchQuery("我爱中国", "title", "content"))
                // 排序条件：置顶
        ...
        // 底层获取得到了高亮显示的值, 但是没有返回. 实际是调用了下面的方法，会在下一个演示
        // class 什么类型的数据
        // 没有使用SearchResultMapper处理
    ...
    public void testSearchByTemplate() {
        ...
        Page<DiscussPost> page = elasticTemplate.queryForPage(searchQuery, DiscussPost.class, new SearchResultMapper() {
            ...
            public <T> AggregatedPage<T> mapResults(SearchResponse response, Class<T> aClass, Pageable pageable) {
                ...
                for (SearchHit hit : hits) {
                    ...
                    // 处理高亮显示的结果
                    // 上面先设置原始的数据是为了防止没有匹配的就会没内容了，其实匹配的在另一个字段里面
        ...
        System.out.println(page.getSize());
        for (DiscussPost post : page) {
            // 返回的是匹配的前后的一段内容，不会将所有的数据都返回，和在网页搜索是一样的
            System.out.println(post);
        }
    }
